...
export const mediaAnalysisSchema = z.object({
  ...
  errors: z.object({ video: z.string().max(2000).optional(), audio: z.string().max(2000).optional() }),
...
export async function checkDetectorHealth(): Promise<DetectorHealth> {
  try {
    ...
    const health = z.object({
      backend: z.literal('UCF+AASIST3'), videoLoaded: z.boolean(),
      audioLoaded: z.boolean(), audioWeightsPresent: z.boolean()
    }).parse(await response.json())
    const voiceReady = health.audioLoaded || health.audioWeightsPresent
    return {
      reachable: true, videoReady: health.videoLoaded, voiceReady,
      message: `Video ${health.videoLoaded ? 'ready' : 'unavailable'} · voice ${voiceReady ? 'assets ready' : 'assets missing'}.`
    }
  } catch {
